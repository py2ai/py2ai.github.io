    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                // Simulate a click at user's location to show info
...
// Update all clock displays (both main and mini)
function updateAllClocks(minutes) {
    ...
    // Mini clock
    const mapClockHand = document.getElementById('map-clock-hand');
    const mapTimeDisplay = document.getElementById('map-time-display');
    const mapClockAmPm = document.getElementById('map-clock-am-pm');
    
    if (mapClockHand) {
        mapClockHand.style.transform = `translateX(-50%) rotate(${degrees}deg)`;
    }
    if (mapTimeDisplay) {
        mapTimeDisplay.textContent = timeStr;
    }
    if (mapClockAmPm) {
        mapClockAmPm.textContent = ampm;
    }
}

// Calculate minutes from mouse/touch position on a clock element
function calculateMinutesFromEvent(e, element) {
...
// Initialize clock sliders (both main and mini)
function initClockSlider() {
    const clockSlider = document.getElementById('clock-slider');
    const mapClockFace = document.getElementById('map-clock-face');
    let isDragging = false;
    let activeElement = null;

    function handleInteraction(e) {
        const minutes = calculateMinutesFromEvent(e, activeElement);
        if (minutes !== null) {
            currentMinutes = minutes;
            updateAllClocks(minutes);
            updateSolarData();
        }
    }
    ...
    // Mini clock events
    if (mapClockFace) {
        mapClockFace.addEventListener('mousedown', (e) => {
            isDragging = true;
            activeElement = mapClockFace;
            handleInteraction(e);
            e.preventDefault();
        });
        
        mapClockFace.addEventListener('touchstart', (e) => {
            isDragging = true;
            activeElement = mapClockFace;
            handleInteraction(e);
            e.preventDefault();
    ...
    // Initial update
    updateAllClocks(currentMinutes);
}
...
let cachedTimezoneLocation = null;

// Get timezone offset for a location using API with fallback
async function getLocationTimezoneOffset(lat, lon, date) {
    // Round coordinates to avoid cache misses for nearby locations
    const roundedLat = Math.round(lat * 100) / 100;
    const roundedLon = Math.round(lon * 100) / 100;
    const cacheKey = `${roundedLat},${roundedLon}`;
    
    // Check cache first
    if (cachedTimezoneLocation === cacheKey && cachedTimezoneOffset !== null) {
    ...
    try {
        const response = await fetch(`https://www.timeapi.io/api/timezone/coordinate?latitude=${roundedLat}&longitude=${roundedLon}`, {
            signal: AbortSignal.timeout(3000)
        });
        if (response.ok) {
            const data = await response.json();
            if (data.currentUtcOffset) {
                offsetMinutes = data.currentUtcOffset.seconds / 60;
            }
        }
    } catch (e) {
    ...
    // Try worldtimeapi.org as backup
    if (offsetMinutes === null) {
        try {
            const response = await fetch(`https://worldtimeapi.org/api/timezone/Auto/${roundedLat},${roundedLon}`, {
                signal: AbortSignal.timeout(3000)
            });
            if (response.ok) {
                const data = await response.json();
    ...
    // If API succeeded, cache and return
    if (offsetMinutes !== null) {
        cachedTimezoneLocation = cacheKey;
        cachedTimezoneOffset = offsetMinutes;
        return offsetMinutes;
    }
    
    // Fallback to manual timezone detection
    return getTimezoneOffsetManual(lat, lon, date);
}

// Manual timezone detection (fallback when API is unavailable)
async function getTimezoneOffsetManual(lat, lon, date) {
    // Nepal (UTC+5:45) - must be checked before India
    if (lon >= 80 && lon <= 89 && lat >= 26 && lat <= 31) {
        return 5.75 * 60; // UTC+5:45, no DST
    }
    
    // Bangladesh (UTC+6) - must be checked before India
    if (lon >= 88 && lon <= 93 && lat >= 20 && lat <= 27) {
        return 6 * 60; // UTC+6, no DST
    }
    
    // Myanmar (UTC+6:30) - must be checked before India
    if (lon >= 92 && lon <= 102 && lat >= 9 && lat <= 29) {
        return 6.5 * 60; // UTC+6:30, no DST
    }
    
    // India (UTC+5:30) - includes Sri Lanka
    if (lon >= 68 && lon <= 97 && lat >= 5 && lat <= 35) {
        return 5.5 * 60; // UTC+5:30, no DST
    }
    
    // Pakistan (UTC+5) - must be checked before Afghanistan
    if (lon >= 60 && lon <= 78 && lat >= 24 && lat <= 37) {
        return 5 * 60; // UTC+5, no DST
    }
    
    // Afghanistan (UTC+4:30) - eastern border ~71°E
    if (lon >= 60 && lon <= 71 && lat >= 29 && lat <= 39) {
        return 4.5 * 60; // UTC+4:30, no DST
    }
...
function setToSunriseFallback(targetLat, targetLon, dateStr) {
    ...
    // Get proper timezone offset instead of longitude-based calculation
    const tzOffset = getTimezoneOffsetSync(targetLat, targetLon, sunrise);
...
function setToSunsetFallback(targetLat, targetLon, dateStr) {
    ...
    // Get proper timezone offset instead of longitude-based calculation
    const tzOffset = getTimezoneOffsetSync(targetLat, targetLon, sunset);
...
function calculateYearlyEnergy(lat, lon, tilt, azimuth) {
    ...
    const totalRatedWattage = config.wattage * totalPanels;
    ...
    const systemEfficiency = config.efficiency / 100;
    ...
    for (let day = 0; day < 365; day++) {
        ...
        for (let hour = 0; hour < 24; hour++) {
            for (let min = 0; min < 60; min += 15) {
                const sunPos = calculateSunPosition(lat, lon, new Date(date.getTime() + hour * 3600000 + min * 60000));
                ...
                const cosIncidence = Math.sin(altitudeRad) * Math.cos(tiltRad) + 
                                    Math.cos(altitudeRad) * Math.sin(tiltRad) * Math.cos(sunAzimuthRad - azimuthRad);
                
                const effectiveIrradiance = irradiance * Math.max(0, cosIncidence);
                const irradianceRatio = effectiveIrradiance / stcIrradiance;
                
                // Power based on rated wattage, capped at max
                const power = (totalRatedWattage * irradianceRatio * systemEfficiency) / 1000;
                const maxPower = totalRatedWattage / 1000;
                const cappedPower = Math.min(power, maxPower);
                
                dayEnergy += cappedPower * 0.25;
            }
        }
        
        totalEnergy += Math.max(0, dayEnergy);
    }
...
function getPanelConfig() {
    ...
    return {
        ...
        efficiency: parseFloat(document.getElementById('panel-efficiency').value) || 85,
...
function calculateCurrentPower(lat, tilt) {
    ...
    // Calculate power based on effective irradiance
    // STC (Standard Test Conditions) = 1000 W/m²
    const stcIrradiance = 1000;
    const irradianceRatio = effectiveIrradiance / stcIrradiance;
    
    // Power is limited by rated wattage
    const totalRatedWattage = config.wattage * totalPanels;
    const power = (totalRatedWattage * irradianceRatio * systemEfficiency) / 1000;
    ...
    return Math.min(Math.max(0, power), maxPower);
}
    ...
    for (let hour = 0; hour < 24; hour++) {
        for (let min = 0; min < 60; min += 15) {
            ...
            if (sunPos.altitude > 0) {
                ...
                // STC (Standard Test Conditions) = 1000 W/m²
                const stcIrradiance = 1000;
                const irradianceRatio = Math.max(0, effectiveIrradiance / stcIrradiance);
...
function calculateAnnualEnergyWithTilt(lat, tilt) {
    ...
    return calculateYearlyEnergy(lat, selectedLocation ? selectedLocation.lon : 0, tilt, azimuth);
}
...
function getTimezoneOffsetSync(lat, lon, date) {
    // Nepal (UTC+5:45) - must be checked before India
    if (lon >= 80 && lon <= 89 && lat >= 26 && lat <= 31) {
        return 5.75; // UTC+5:45, no DST
    }
    
    // Bangladesh (UTC+6) - must be checked before India
    if (lon >= 88 && lon <= 93 && lat >= 20 && lat <= 27) {
        return 6; // UTC+6, no DST
    }
    
    // Myanmar (UTC+6:30) - must be checked before India
    if (lon >= 92 && lon <= 102 && lat >= 9 && lat <= 29) {
        return 6.5; // UTC+6:30, no DST
    }
    ...
    // Pakistan (UTC+5) - must be checked before Afghanistan
    if (lon >= 60 && lon <= 78 && lat >= 24 && lat <= 37) {
        return 5; // UTC+5, no DST
    }
    
    // Afghanistan (UTC+4:30) - eastern border ~71°E
    if (lon >= 60 && lon <= 71 && lat >= 29 && lat <= 39) {
        return 4.5; // UTC+4:30, no DST
    }
